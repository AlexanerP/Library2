...
public class OrderStatusImpl extends DAOHelper implements StatusDAO {

    private final static Logger logger = LoggerFactory.getLogger(OrderStatusImpl.class);
    ...
    private final static String DELETE_QUERY = String.format("DELETE FROM %s WHERE %s=?", TableName.ORDER_STATUS,
            ColumnName.ORDER_STATUS_ID_STATUS);

    private final static String UPDATE_QUERY = String.format("UPDATE %s SET %s=? WHERE %s=?",
            TableName.ORDER_STATUS, ColumnName.ORDER_STATUS_STATUS, ColumnName.ORDER_STATUS_ID_STATUS);

    private final static String GET_STATUS_BY_ID_QUERY = String.format("SELECT * FROM %s WHERE %s=?;",
            TableName.ORDER_STATUS, ColumnName.ORDER_STATUS_ID_STATUS);

    private final static String GET_ALL_STATUS_QUERY = String.format("SELECT * FROM %s;", TableName.ORDER_STATUS);

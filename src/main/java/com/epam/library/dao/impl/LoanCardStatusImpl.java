...
public class LoanCardStatusImpl extends DAOHelper implements StatusDAO {

    private final static Logger logger = LoggerFactory.getLogger(LoanCardStatusImpl.class);
    ...
    private final static String DELETE_QUERY = String.format("DELETE FROM %s WHERE %s=?", TableName.LOAN_CARDS_STATUS,
            ColumnName.CARD_STATUS_ID_STATUS);

    private final static String UPDATE_QUERY = String.format("UPDATE %s SET %s=? WHERE %s=?",
            TableName.LOAN_CARDS_STATUS, ColumnName.CARD_STATUS_STATUS, ColumnName.CARD_STATUS_ID_STATUS);

    private final static String GET_STATUS_BY_ID_QUERY = String.format("SELECT * FROM %s WHERE %s=?;",
            TableName.LOAN_CARDS_STATUS, ColumnName.CARD_STATUS_ID_STATUS);

    private final static String GET_ALL_STATUS_QUERY = String.format("SELECT * FROM %s;", TableName.LOAN_CARDS_STATUS);
